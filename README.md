...
| 移动端 | 左侧虚拟摇杆 | 右侧四向按钮 | 悬浮玻璃按钮放炸弹 / 用主动道具 · 点击暂停界面继续 |
...
移动端按横屏设计：竖屏会弹出转屏提示并自动暂停，横过来即可继续；安卓端首次触摸
会尝试全屏并锁定横屏。全部操作按钮以 Apple 风格的半透明拟态玻璃悬浮在画面上——
左侧摇杆移动、右侧四向按钮射击，四向按钮旁另有「炸弹」「道具」两枚圆形玻璃按钮，
补齐了桌面端 E 键和空格的能力。
...
style.css              布局 / 玻璃拟态触控按钮 / 竖屏转屏提示
js/utils.js            数学工具 / 常量 / 种子随机
js/meta.js             元进度（解锁 / 角色定义 / localStorage 存档）
js/char-powers.js      角色专属机制（怒气 / 魂火 / 恶魔定价 / 赌徒运势）
js/audio.js            WebAudio 合成音效
js/sprites-room.js     房间绘制（石墙与血肉章节烘焙、岩石、门、配色）
js/sprites-actors.js   角色与敌人绘制（dodo / 翅膀 / 眼泪 / 激光）
js/sprites-items.js    拾取物 / 道具座 / 商店 / 89 件道具图标
js/sprites-fx.js       后期特效（暗角 / 颗粒）、蜡笔文字、HUD
js/items.js            89 件道具定义与效果
js/dungeon.js          随机地牢生成 + 小地图
js/entities-player.js  玩家 / 眼泪 / Brimstone 激光 / 爆炸 / 粒子
js/entities-enemies.js 敌人 AI / 敌方弹幕与激光 / 受击结算
js/boss-core.js        13 个 Boss 定义、血量曲线、状态机
js/boss-attacks.js     通用攻击行为库
js/boss-attacks-signature.js  各 Boss 专属招式（每招只属于一个 Boss）
js/boss-render.js      Boss 绘制
js/leaderboard.js      全服排行榜（本地打开自动隐藏）
js/menu-unlocks.js     环形选人 / 解锁图鉴（I 键，局内可随时查看）/ 解锁弹窗
js/game-core.js        全局状态 / 输入 / 暂停 / 触控
js/game-flow.js        局与楼层流程（开局 / 进房 / 清房 / 下层）
js/game-update.js      每帧逻辑更新
