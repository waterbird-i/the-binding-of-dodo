...
整个游戏由 AI 编程完成：从地牢生成算法、14 个 Boss 的状态机，到 89 件道具的程序化图标和排行榜后端对接，我只负责提需求和验收。逻辑问题靠 `test/playtest.cjs` 里的 360 条断言抓：真键盘事件驱动 file:// 页面，对活的游戏状态做检查，凡是「改了个函数名忘了改调用点」这种错，几秒内就会翻车。美术问题断言抓不住，只能出图人工看，本 README 的所有截图和动图也是同一套摆拍脚本产出的。
...
游戏本体约 10200 行（js 9931 + css 166 + html 66），测试另有约 3600 行。
