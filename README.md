...
秘密房、献祭房、第 3 层起的小 Boss 房看运气。八个视觉章节各有独立配色和地面烘焙，
...
| 档 | 敌人血量 | 刷怪数 | 敌人速度 | 接触/弹幕伤害 | Boss 血量窗口 | Boss 出招间隔 | 弹速 | 受击无敌帧 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 轻松 | ×0.6 | ×0.70 | ×0.85 | ×0.6 | 44s / 6.4s | ×1.5 | ×0.82 | ×1.2 |
| 标准 | ×1 | ×1 | ×1 | ×1 | 55s / 8s | ×1 | ×1 | ×1 |
| 硬核 | ×1.25 | ×1.15 | ×1.08 | ×1.25 | 63.3s / 10.4s | ×0.88 | ×1.08 | ×1 |

「Boss 血量窗口」一列是「上限 / 下限」，单位是你站桩输出的秒数——打再慢也不会超过 55 秒
（硬核 63.3 秒），打再快也得磨 8 秒（硬核 10.4 秒），除非输出已经超过血表 ×1.5。

轻松档另外有两条只作用于开局的规则：第 1、2 层的敌人血量 / 速度 / 伤害 / 弹速再乘 0.7、
0.7、0.7、0.85（第 1 层 gaper 血量 12 → 5.0，速度 80 → 48 px/s，每房小怪 2.9 → 1.9 只），
清房掉半颗心的概率从 6% 提到 18%（第 3 层起回到 6%）。第 1 层那场教学 Boss 也从 300 血
降到 195 血——裸装 dodo 只有 8.3 DPS，原样要站桩硬射 36 秒，现在 23 秒。标准 / 硬核的
第一个 Boss 仍是 300 血，开局的敌人与回血规则也照旧。
...
- 轻松档现在明显比标准档松（敌人血量 ×0.6、伤害 ×0.6、前两层再叠一层开局宽容、第 1 层
  Boss 195 血），代价是后期也基本没有压力，属于「想先把 12 层走完」的档。两档之间没有
  中间选项：标准是原始手感，硬核是给摸熟了的人准备的。第 1、2 层不会出现小 Boss，第 3 层
  起才有。
